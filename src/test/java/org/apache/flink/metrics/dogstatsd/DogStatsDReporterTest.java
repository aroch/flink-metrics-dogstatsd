...
public class DogStatsDReporterTest extends TestLogger {
    ...
    private static ArrayList<String> expectedConfigTags = new ArrayList<>();

    static {
        expectedConfigTags.add("bla:bli");
    }
    ...
    public void testAddingMetrics() throws Exception {
        ...
        MetricRegistryImpl metricRegistry = new MetricRegistryImpl(
                ...
                Collections.singletonList(ReporterSetup.forReporter("test", new TestingDogStatsDReporter())));
        ...
        assertTrue("Reporter should be of type DatadogStatsDReporter", metricReporter instanceof DogStatsDReporter);

        TestingDogStatsDReporter reporter = (TestingDogStatsDReporter) metricReporter;

        Map<Counter, DCounter> counters = reporter.getCounters();
        ...
        DCounter dCounter = counters.get(myCounter);

        assertEquals(expectedCounterName, dCounter.getMetric());

        List<String> expectedTags = new ArrayList<>();

        for (Map.Entry<String, String> entry : taskMetricGroup.getAllVariables().entrySet()) {
            expectedTags.add(entry.getKey().substring(1, entry.getKey().length() - 1) + ":" + entry.getValue());
        }
        expectedTags.addAll(expectedConfigTags);

        assertEquals(new HashSet<>(expectedTags), new HashSet<>(dCounter.getTags()));
    ...
    public static class TestingDogStatsDReporter extends DogStatsDReporter {
        ...
        public void open(MetricConfig configuration) {
            ...
            configTags = new ArrayList<>(expectedConfigTags);
        }

        public Map<Counter, DCounter> getCounters() {
            return counters;
        }
    }
